<template>
  <div class="container">
    <page-header title="DAI 币" @goback="router.go(-1)" />
    <van-form label-width="60" @submit="onSubmit">
      <van-cell-group title="Hayek鏈上資產">
        <van-cell title="DAI@hayek" :value="formData1.balance">
          <template #icon>
      ...
      <div class="footer">
        <van-button type="danger" native-type="submit" size="small" @click="ttype=1">轉帳到Polygon</van-button>
        <div style="width: 20px;"></div>
        <van-button type="primary" native-type="submit" size="small" @click="ttype=2">Hayek鏈轉帳</van-button>
      </div>
    </van-form>
    <van-form label-width="60" @submit="onSubmit">
      <van-cell-group title="Polygon鏈上資產">
        <van-cell title="DAI@Polygon" :value="formData2.balance">
          <template #icon>
            <img width="16" height="16" style="margin-top: 3px;" src="data:image/png;base64," />
          </template>
        </van-cell>
...
import { balanceOfDAI, balanceOfDaiOnPolygon, hayekTransfer, polygonTransfer } from '@/utils/tokenHelper';
...
onMounted(async() => {
  ...
  if(window.polygon){
    balanceOfDaiOnPolygon(appData.wallet).then(value =>{
